...
void loadSettings(Settings &settings) {
  ...
  settings.port = doc["port"] | 2731;
  strlcpy(settings.hostname, doc["hostname"] | "example.com", sizeof(settings.hostname));
  strlcpy(settings.ssid, doc["ssid"] | "devnet-34", sizeof(settings.ssid));
  strlcpy(settings.wkey, doc["wkey"] | "testerwlan", sizeof(settings.wkey));
  file.close();
}
